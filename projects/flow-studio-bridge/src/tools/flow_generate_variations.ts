...
export function registerFlowGenerateVariations(server: McpServer, runtime: FlowRuntime): void {
  server.tool(
    ...
    async (params) => {
      ...
      const result = await runtime.queue.run(runtime.config, "flow_generate_variations", async () => {
        if (params.dryRun) {
          return generateVariations(page, runtime.config, params);
        }
        return runVariationsJob(page, runtime.config, params);
      });
